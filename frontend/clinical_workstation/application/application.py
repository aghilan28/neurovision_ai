"""Assemble the Clinical Workstation view-model from a loaded snapshot.
...
validation, and returns a single :class:`WorkstationView`. Everything it returns
is derived from registered artifacts in the snapshot (presentation only).
"""
...
def build_workstation_view(state: WorkstationState) -> WorkstationView:
    """Build the full, presentation-only workstation view-model."""
    state.default_context()
    areas = build_areas(state)
    validation = validate_state(state).to_dict()
    meta = {
        "workstation_version": CLINICAL_WORKSTATION_VERSION,
