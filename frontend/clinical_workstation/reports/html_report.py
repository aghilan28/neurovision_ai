"""Deterministic static-HTML renderer for the Clinical Workstation.

Produces a single self-contained HTML page (inline CSS + inline SVG, CSS-only
nav, no JavaScript, no external assets) so the unified workflow environment works
...
from ..version import CLINICAL_WORKSTATION_VERSION
...
def _line_svg(spec: dict) -> str:
    ...
    body = [f"<rect x='{pad}' y='{pad}' width='{inner_w}' height='{inner_h}' fill='none' stroke='#30363d'/>"]
    coords = []
    for p in pts:
        x = p.get("x"); y = p.get("y")
        if x is None or y is None:
            continue
        px = pad + float(x) * inner_w
        py = (H - pad) - float(y) * inner_h
        coords.append((px, py))
...
def render_workstation_html(view: WorkstationView) -> str:
    ...
    return (
        ...
        f"<title>NeuroVision AI — Clinical Workstation</title><style>{_CSS}{reveal}</style></head><body>"
        "<header><h1>NeuroVision AI — Clinical Workstation</h1>"
        f"<div class='sub mono'>{CLINICAL_WORKSTATION_VERSION} · "
        f"cases {_esc(meta.get('n_cases'))} · reviews {_esc(meta.get('n_reviews'))} · "
        f"findings {_esc(meta.get('n_findings'))} · "
        f"validation: <span class='badge {vb}'>{'OK' if val.get('ok') else 'FAILED'}</span></div>"
        f"<div class='sub'>context: case={_esc(ctx.get('current_case'))} "
        f"review={_esc(ctx.get('current_review'))} finding={_esc(ctx.get('current_finding'))}</div>"
