"""Design-system static HTML renderer for the Clinical Workstation."""
...
def render_workstation_html(view: WorkstationView) -> str:
    """Render the workstation view-model to deterministic clinical HTML."""
    return render_workstation_view(
        view,
        title="NeuroVision Clinical Workstation",
        subtitle="Hospital-grade intelligence environment",
        version=CLINICAL_WORKSTATION_VERSION,
    )
...
def write_workstation_html(snapshot_path: str, out_path: str | None = None) -> str:
    ...
    out_path = out_path or os.path.join(
        os.path.dirname(os.path.abspath(snapshot_path)), "clinical_workstation.html"
    )
