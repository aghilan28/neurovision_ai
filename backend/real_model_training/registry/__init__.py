"""``backend/real_model_training/registry`` — real trained-model registry (T2).

Tracks the dataset / recording / feature-asset / training-run / model / evaluation /
benchmark / readiness / comparison entities. **No orphan records**: every entry must
reference an audit head and a lineage node.
"""

from __future__ import annotations

from ..models.domain import EntityKind, TrainingRegistryRecord
...
class RealModelRegistry:
    def __init__(self) -> None:
        self._records: dict[str, TrainingRegistryRecord] = {}
        self._sigs: dict[tuple, str] = {}

    def register(self, record: TrainingRegistryRecord) -> TrainingRegistryRecord:
    ...
    def get(self, entity_id: str) -> TrainingRegistryRecord:
...
__all__ = ["RealModelRegistry", "RegistryError", "GENESIS"]
