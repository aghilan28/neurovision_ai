"""Task dependency analysis (V4-P4).

Deterministic, read-only helpers over the registered task dependencies. The
dependency graph is *structure* — it records how tasks relate (depends_on, blocks,
supports, requires, derived_from, influences); it never executes anything. The
analyzer detects cycles among ``depends_on``/``requires`` edges (a defect) so the
validation layer can flag a structurally invalid task graph. All ordering is
...
def build_adjacency(dependencies: Iterable) -> dict:
    """Task-id -> sorted list of task-ids it depends_on/requires (task targets only)."""
    adj: dict = {}
    for d in dependencies:
        if d.relation in _ORDERING_RELATIONS and d.target_kind == "task":
            adj.setdefault(d.source_task_id, set()).add(d.target_id)
