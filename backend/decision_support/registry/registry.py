"""The decision registry: governed, versioned, traceable decision-support artifacts.

No decision-support artifact may exist outside the registry. A new artifact
*version* with the same id is an update; re-registering the *same* version with
different content is a forbidden silent overwrite.
"""

from __future__ import annotations

from ..version import DECISION_REGISTRY_VERSION
from ..models.domain import DecisionRegistryRecord
...
class DecisionRegistry:
    ...
    def __init__(self) -> None:
        self._records: dict[str, DecisionRegistryRecord] = {}
        self._version_sigs: dict[tuple[str, str], str] = {}

    def register(self, record: DecisionRegistryRecord) -> DecisionRegistryRecord:
        ...
        if key in self._version_sigs and self._version_sigs[key] != sig:
            raise ValueError(
                f"decision artifact {record.artifact_id} version {record.version} already "
                "registered with different content (silent overwrite forbidden)")
        self._version_sigs[key] = sig
        self._records[record.artifact_id] = record
        return record

    def get(self, artifact_id: str) -> DecisionRegistryRecord:
        if artifact_id not in self._records:
            raise KeyError(f"decision artifact {artifact_id!r} not in registry")
    ...
    def to_dict(self) -> dict:
        return {"decision_registry_version": DECISION_REGISTRY_VERSION,
