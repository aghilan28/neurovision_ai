"""``backend/operations_platform/registry`` — operations registry (T4-A/G).

Tracks health checks / metrics snapshots / diagnostics / qualifications / readiness
assessments. **No orphan records**: every entry must reference an audit head and a lineage
node.
"""

from __future__ import annotations

from ..models.domain import EntityKind, OperationsRegistryRecord
...
class OperationsRegistry:
    def __init__(self) -> None:
        self._records: dict[str, OperationsRegistryRecord] = {}
        self._sigs: dict[tuple, str] = {}

    def register(self, record: OperationsRegistryRecord) -> OperationsRegistryRecord:
    ...
    def get(self, entity_id: str) -> OperationsRegistryRecord:
...
__all__ = ["OperationsRegistry", "RegistryError", "GENESIS"]
