"""Deterministic serving identity generation, policy, and validation.

An identity is ``"{kind}+{hash16}"`` (the platform-wide scheme, NR-6). This layer mints
the serving-specific kinds — ``serving_request``, ``serving_execution``,
``serving_response``, ``serving_readiness`` — each content-addressed from its inputs (so
identical inputs reproduce identical ids). The upstream ``model``/``prediction``/
``feature``/``dataset`` ids are minted by their owning subsystems (reused, not duplicated);
this layer *validates* (but never mints) the ids it references.
"""
...
from ..version import SERVING_IDENTITY_VERSION

_ID_RE = re.compile(
    r"^(serving_request|serving_execution|serving_response|serving_readiness"
    r"|model|prediction|feature|dataset)"
    r"\+[0-9a-f]{16}$")
...
class IdentityPolicy:
    ...
    mintable: bool = True
    identity_version: str = SERVING_IDENTITY_VERSION


IDENTITY_POLICIES: dict[str, IdentityPolicy] = {
    "serving_request": IdentityPolicy(
        "serving_request", ("model_ref_key", "feature_asset_id", "input_key"), None, None),
    "serving_execution": IdentityPolicy(
        "serving_execution", ("request_id", "prediction_id"), "serving_request", "request_id"),
    "serving_response": IdentityPolicy(
        "serving_response", ("execution_id", "response_key"), "serving_execution", "execution_id"),
    "serving_readiness": IdentityPolicy(
        "serving_readiness", ("target_id", "readiness_key"), None, None),
    # Referenced-only kinds (minted upstream); present to *validate* them.
